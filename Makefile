ifeq ($(OS), Windows_NT)
    ...
    RM := del /q
    CLEAN_PATH := src\lib\glad\src\glad.o src\main.o src\gui\*.o src\core\*.o src\misc\*.o src\state\*.o $(EXEC)
else
    # LINUX SETTINGS
    EXEC := a.out
...
SRCS := src/main.c $(wildcard src/gui/*.c src/core/*.c src/misc/*.c src/state/*.c) $(GLAD_PATH)
OBJS := $(SRCS:.c=.o)
